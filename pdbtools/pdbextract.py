...
class PDBExtract(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.extractseq or self.excludeseq:
            ...
            (self.querySeqFastaName, self.querySeqFasta) = \
                readFasta(filename)
        return
    ...
    def newFileName(self, filename, includedChain):
        if self.overwrite:
            ...
            newFileName = filename[:len(filename) - 4] + '_' \
                + includedChain + '.pdb'
        return newFileName
    ...
    def extractRegions(
        ...
        for region in regions:
            ...
            if match:
                ...
                regionDictionary[match.group(1)] = \
                    '{0}_{1}'.format(start, end)
                includedChain = includedChain + match.group(1)
        buffer = filterATOM(chain, regionDictionary)

        if len(buffer) > 0:

            newFileName = self.newFileName(filename,
                    ''.join(regionDictionary.values()))
            f = open(newFileName, 'w')
            if self.header:
                filteredHeader = filterHeader(header,
                        includedChain.split())
    ...
    def splitChains(
        ...
        for chain in chains:
            newFileName = filename[:len(filename) - 4] + '_' + chain \
                + '.pdb'
            f = open(newFileName, 'w')
            if self.header:
    ...
    def extractBasedOnStructureSeq(self, querysequence, header, chains, selectedchain, filename):
        '''
        Extract portion of structure matched with given sequence(querysequence) and chain name (selectedchain)
        ...
        sequence = ""
        resnum = []
        for line in chains[selectedchain]:
            atomtype = line[12:16].strip()
            if line[17:20] in oneLetter and atomtype=="CA":
                sequence = sequence+oneLetter[line[17:20]]
                resnum.append(int(line[22:26]))
        gapOpen = -10
        gapExtend = -0.5
        matrix = matlist.blosum62
        aln = pairwise2.align.globalds(querysequence,sequence, matrix,\
                                         gapOpen, gapExtend)
        ...
        temp=subject[:start].replace("-","")
        adjustedStart = len(temp)
        temp=subject[:end].replace("-","")
        adjustedEnd = len(temp)
    ...
    def extractBasedOnSeq(self, header, chains, filename):
        '''
        Extract pdb chain based on the SEQRES header content. If pdb file lacks SEQRES header, it would not work. 
        In that cases, use extractBasedOnStructureSeq.
        ...
        if len(sequence) > 1:
            ...
            for chain in sequence:
                ...
                aln = pairwise2.align.globalds(self.querySeqFasta,
                        sequence[chain], matrix, gapOpen, gapExtend)
                (query, subject, score, begin, end) = aln[0]
            ...
            newFileName = self.saveChains(chains, header, includeList,
                    filename)
        else:
            newFileName = ''
        return newFileName
    ...
    def uniqueChain(
        ...
        for i in range(len(sequences)):
            for j in range(i + 1, len(sequences)):
                aln = pairwise2.align.globalds(sequences[i],
                        sequences[j], matrix, gapOpen, gapExtend)
                (query, subject, score, begin, end) = aln[0]
    ...
    #
        ...
        if self.extractseq or self.excludeseq:
            newFileName = self.extractBasedOnSeq(header, chains,
                    filename)

        if self.unique:
            newFileName = self.uniqueChain(header, chains, filename)

        if (self.hetero or self.filter) and not self.exclude \
            and not self.unique and not self.extract and not self.split:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--remove_residue', action='store',
                        dest='filter',
    ...
    group.add_argument('-R', '--extract_region', action='store',
                       dest='extractregion',
    ...
    group.add_argument('-E', '--extract_sequence', action='store',
                       dest='extractseq',
                       help='Extract PDB chains based on the protein sequences'
                       )
    group.add_argument('-X', '--exclude_sequence', action='store',
                       dest='excludeseq',
    ...
    pdbExtract = PDBExtract(
        ...
        extractregion=results.extractregion,
        ...
        extractseq=results.extractseq,
        excludeseq=results.excludeseq,
        )
...
if __name__ == "__main__":
	...
	fileList = pdbExtract.go()
>>>>>>> d8497215216193548849b96ef1ab5fd7e31c2b7d
